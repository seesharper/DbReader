...
public static void PatchAssemblyVersion(string version, string pathToAssemblyInfoFile)
{
    string tempFile = Path.GetTempFileName();

    using(StreamReader reader = new StreamReader(pathToAssemblyInfoFile))
    {
        string assemblyInfo = reader.ReadToEnd();
        var patchedAssemblyInfo = Regex.Replace(assemblyInfo, @"Version\(""(.+?"")", string.Format(@"Version(""{0}""", version));

        using(StreamWriter writer = new StreamWriter(tempFile))
        {
            writer.Write(patchedAssemblyInfo);
        }    
    }
...
public static void PatchAssemblyVersionFromNugetSpecification(string pathToNugetSpecification, string pathToAssemblyInfoFile)
{
    string nugetVersionNumber = GetNugetVersionNumber(pathToNugetSpecification);
    PatchAssemblyVersion(nugetVersionNumber, pathToAssemblyInfoFile);
}

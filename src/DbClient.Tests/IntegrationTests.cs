...
namespace DbClient.Tests
{
    ...
    public class IntegrationTests
    {
        ...
        static IntegrationTests()
        {
            ...
            DbClientOptions.WhenPassing<CustomValueType>()
                .Use((parameter, argument) =>
                {
                    parameter.Value = argument.Value;
                });

            DbClientOptions.WhenReading<CustomScalarValue>().Use((datarecord, i) => new CustomScalarValue(datarecord.GetInt64(i)));
        }
        ...
        {
            DbClientOptions.KeySelector<CustomerWithCustomKeySelector>(c => c.CustomerId);
            using (var connection = CreateConnection())
            {
                var customers = await connection.ReadAsync<Customer>(SQL.Customers);
                customers.Count().ShouldBe(93);
        ...
        public void ShouldReadCustomersAndOrdersAsRecordType()
        {
            ...
            // DbClientOptions.KeySelector<CustomerRecordWithOrders>(c => c.CustomerId);
            // DbClientOptions.KeySelector<OrderRecord>(o => o.OrderId);
        ...
        {
            bool invoked = false;
            DbClientOptions.CommandInitializer = command => { invoked = true; };
            ...
            DbClientOptions.CommandInitializer = null;
        }
        ...
        {
            DbClientOptions.WhenPassing<CustomerId>().Use((p, c) =>
                {
        ...
        {
            DbClientOptions.WhenReading<CustomValueType>().Use((record, ordinal) => new CustomValueType(record.GetInt32(ordinal)));
        ...
        public void GetAllCustomersUsingDbClient()
        {
            using (var connection = CreateConnection())
            {
                var dbClientResult = Measure.Run(() => connection.Read<Customer>(SQL.Customers), 10,
                    "DbClient (All Customers)");
                Console.WriteLine(dbClientResult);
            }
        }
